...
"" 4. <ascii_mode> : (Optional) If provided and set to 1, the program will map the decrypted 
""    point back to ASCII characters. This mode is only valid if the ciphertext was generated
""    using message passing in Encrypt.py (Refer to documentation).
""
...
USAGE1 = "sage Decrypt.py <priv_key> <pub_key> <ciphertext>"
USAGE2 = "sage Decrypt.py <priv_key> <pub_key> <ciphertext> <ascii_mode>"
if (len(sys.argv) < 4):
...
def main():
    ...
    if sys.argv.__len__() == 5 and sys.argv[4] == '1':
